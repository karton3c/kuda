...
def gen_net_c(node, interp_eval_fn):
    ...
    L.append(f'            /* hidden deltas */')
    L.append(f'            int w_off_arr[{NAME}_N_LAYERS];')
    L.append(f'            w_off_arr[0] = 0;')
    L.append(f'            for(int li=1; li<{name}_n_layers-1; li++)')
    L.append(f'                w_off_arr[li] = w_off_arr[li-1] + {name}_layers[li-1]*{name}_layers[li];')
    L.append(f'            for(int li={name}_n_layers-2; li>0; li--) {{')
    L.append(f'                int cur={name}_layers[li], nxt={name}_layers[li+1];')
    L.append(f'                int wo = w_off_arr[li];')
